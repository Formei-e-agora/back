...
const fs = require('fs');
const { generateHtmlJobList } = require('./generateHtmlListTable');
...
function sendNewestJobs(userEmail, jobs, name) {
  ...
  const htmlList = generateHtmlJobList(jobs);

  if (htmlList !== '') {
    const mailSubject = 'Novas Oportunidades';
    let mailBody = fs.readFileSync(`${templatePath}/newestJobs.html`, { encoding: 'utf-8', flag: 'r' });
    mailBody = mailBody.replace('#NOME_DO_CONTATO', name);
    mailBody = mailBody.replace('#LISTA_DE_OPORTUNIDADES', htmlList);
